...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=para \
...
build-docker:
	...
	$(DOCKER) create --name para -t -i ${DOCKER_IMAGE}:latest para
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp para:/usr/bin/parad ./build/
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(EVMOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/para:Z para/node "./parad testnet init-files --v 4 -o /para --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\parad:/evmos\Z evmos/node "./parad tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node1\parad:/evmos\Z evmos/node "./parad tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node2\parad:/evmos\Z evmos/node "./parad tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node3\parad:/evmos\Z evmos/node "./parad tendermint unsafe-reset-all --home=/evmos"
else
	@docker run --rm -v $(CURDIR)/build/node0/parad:/evmos:Z evmos/node "./parad tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node1/parad:/evmos:Z evmos/node "./parad tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node2/parad:/evmos:Z evmos/node "./parad tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node3/parad:/evmos:Z evmos/node "./parad tendermint unsafe-reset-all --home=/evmos"
endif
